...
def www_choose_regular_question_radio_button_handler(obj: MainWindow) -> None:
    ...
    _initialize_www_timer(obj, initial_time=obj.app_config.www_regular_time)
    _initialize_main_window_www_timer_widget(obj)
    _initialize_game_window_www_timer_widget(obj)
...
def www_choose_blitz_radio_button_handler(obj: MainWindow) -> None:
    ...
    _initialize_www_timer(obj, initial_time=obj.app_config.www_blitz_time)
    _initialize_main_window_www_timer_widget(obj)
    _initialize_game_window_www_timer_widget(obj)
    if isinstance(obj.current_game, WWWGame):
...
def www_choose_super_blitz_radio_button_handler(obj: MainWindow) -> None:
    ...
    _initialize_www_timer(obj, initial_time=obj.app_config.www_super_blitz_time)
    _initialize_main_window_www_timer_widget(obj)
    _initialize_game_window_www_timer_widget(obj)
    if isinstance(obj.current_game, WWWGame):
    ...
    if all([
        ...
        and any([
            (
                ...
                or obj.current_game.current_super_blitz_question == WWWSuperBlitzQuestionCounter.THIRD
            ),
            (
                obj.current_game.current_super_blitz_question is None
                and obj.current_game.current_blitz_question is None
            ),
        ]),
    ]):
        _run_timer_to_provide_answers_before_finish_round(obj=obj)
        return
    _finish_round_immediately(obj=obj)


def www_timer_timer_ten_seconds_left_handler(obj: MainWindow) -> None:
    """Обработчик события: на таймере осталось 10 секунд (ЧГК)."""
    if obj.current_game.round_mode == WWWRoundModeEnum.REGULAR:
        _play_sound_in_www_handlers(obj, sound=SoundFileEnum.TIMER_10_SEC_LEFT)
...
def _run_timer_to_provide_answers_before_finish_round(obj: MainWindow) -> None:
...
def _finish_round_immediately(obj: MainWindow) -> None:  # noqa: WPS231
    """Завершает раунд, если не предусмотрено доп. времени на сбор ответов игроков."""
    match obj.current_game.round_mode:
        case WWWRoundModeEnum.REGULAR:
            _play_sound_in_www_handlers(obj, sound=SoundFileEnum.TIMER_START_STOP)
...
def _initialize_www_timer(obj: MainWindow, initial_time: int, precision: int = 100) -> None:
...
def _play_sound_in_www_handlers(obj: MainWindow, sound: SoundFileEnum) -> None:
...
def _initialize_main_window_www_timer_widget(obj: MainWindow) -> None:
...
def _initialize_game_window_www_timer_widget(obj: MainWindow) -> None:
