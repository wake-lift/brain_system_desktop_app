...
def brain_ring_player_key_press_handler(obj: MainWindow, player: Player):
    ...
    if obj.current_game.status == BrainRingGameStatusEnum.COUNTDOWN_STARTED:
        _handle_brain_player_who_gives_answer(obj=obj, player=player)
    elif obj.current_game.status == BrainRingGameStatusEnum.PLAYER_BUTTON_PRESSED:
        _handle_brain_player_who_was_not_first_to_answer(obj=obj, player=player)
    elif all([
        obj.current_game.status == BrainRingGameStatusEnum.READY_TO_START_COUNTDOWN,
        not obj.current_game.is_false_start_active,
    ]):
        _handle_brain_player_who_made_a_false_start(obj=obj, player=player)
...
def _handle_brain_player_who_gives_answer(obj: MainWindow, player: Player) -> None:
    ...
    # добавляем виджет на панель ведущего с информацией о заблокированных игроках
...
def _handle_brain_player_who_was_not_first_to_answer(obj: MainWindow, player: Player) -> None:
...
def _handle_brain_player_who_made_a_false_start(obj: MainWindow, player: Player) -> None:
