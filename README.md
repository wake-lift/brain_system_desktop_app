...
- Для упаковки приложения используется библиотека `nuitka`.
- Перед началом компиляции необходимо убедиться, что путь к директории проекта не содержит кириллических символов.

Сборка portable-версии с среде Linux осуществляется командой:
```
nuitka --standalone --assume-yes-for-downloads --include-data-dir=assets=assets --include-data-files=config.ini=config.ini --enable-plugin=pyqt6 --include-qt-plugins=multimedia --lto=yes --remove-output --output-filename=brain_system_app main.py
```
В этой команде:
- флаг `--standalone` - создает один независимый бинарник со всеми зависимостями;
- флаг `--assume-yes-for-downloads` - неинтерактивная сборка (на все вопросы автоматически отвечать «да»);
- флаг `--include-data-dir` - копирует каталог `assets` файл конфигурации `config.ini` в бинарник;
- флаги `--enable-plugin=pyqt6` и `--include-qt-plugins=multimedia` - обеспечивают корректное подключение плагина PyQt6;
- флаг `--lto=yes` - включает link-time optimization (уменьшает размер и ускоряет запуск приложения);
- флаг `--remove-output` - удаляет временную папку сборки после компиляции;
- флаг `--output-filename` - задает название скомпилированного бинарного файла.

Сборка portable-версии для запуска в среде Windows осуществляется командой:
```
python -m nuitka --standalone --assume-yes-for-downloads --include-data-dir=assets=assets --include-data-files=config.ini=config.ini   --enable-plugin=pyqt6 --include-qt-plugins=multimedia --lto=yes --remove-output --windows-console-mode=disable --windows-icon-from-ico=assets/icons/bulb.ico --output-filename=brain_system_app main.py
```
Дополнительные опции в этой команде:
- флаг `--windows-icon-from-ico=assets/icons/bulb.ico` - добавляет иконку для exe-файла (на Linux/MacOS этот флаг будет проигнорирован).
- флаг `--windows-console-mode=disable` - отключает отображение консоли при запуске приложения.

Готовая сборка будет находиться в директории `main.dist`.
